...
export default function CartSidebar() {
    ...
    return (
        <aside className=" fixed right-10 top-20">
            {isOpen ? (
                <div className="w-72 bg-slate-900 p-4  text-white">
                    <h2 className="text-xl font-bold mb-4">Your Cart</h2>
                     <svg onClick={()=>setIsOpen(false)}
                        className="fixed right-15 cursor-pointer top-25"
                        viewBox="-0.5 0 25 25"
                        width={25}
                        fill="none"
                    ...
                    {cart?.items?.length === 0 ? (
                        <div className="text-gray-400 p-4">Cart empty</div>
                    ) : (
                        <>
                            {cart?.items?.map((item) => (
                                <div
                            ...
                            <Link
                                href="/cart"
                                className="block mt-4 text-center bg-indigo-600 py-2 rounded"
                            >
                                View Cart
                            </Link>
                        </>
                ...
                <button onClick={()=>setIsOpen(true)} className=" bg-blue-500 px-2 py-2 text-white rounded-4xl">
                    <svg
                        className=""
                        viewBox="-0.5 0 25 25"
                        width={35}
